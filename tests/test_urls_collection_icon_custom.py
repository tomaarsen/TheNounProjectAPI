...
class CollectionIconsCustomURLs(unittest.TestCase):
    ...
    def _test_get_collection_icons(self, identifier, limit=None, offset=None, page=None):
        """
        Helper function to call tnp's get_collection_icons in such a way that we only get the URL
        and don't actually make the request.
        """
        return self.tnp.get_collection_icons(identifier, limit=limit, offset=offset, page=page).url
