...
class CollectionIconsURLs(unittest.TestCase):
    ...
    def _test_get_collection_icons_by_id(self, _id, limit=None, offset=None, page=None):
        """
        Helper function to call tnp's get_collection_icons_by_id in such a way that we only get the URL
        and don't actually make the request.
        """
        return self.tnp.get_collection_icons_by_id(_id, limit=limit, offset=offset, page=page).url
    ...
    def _test_get_collection_icons_by_slug(self, slug, limit=None, offset=None, page=None):
        """
        Helper function to call tnp's get_collection_icons_by_slug in such a way that we only get the URL
        and don't actually make the request.
        """
        return self.tnp.get_collection_icons_by_slug(slug, limit=limit, offset=offset, page=page).url
