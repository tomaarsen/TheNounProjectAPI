...
class CollectionsURLs(unittest.TestCase):
    ...
    def _test_get_collections(self, limit=None, offset=None, page=None):
        """
        Helper function to call tnp's get_collections in such a way that we only get the URL
        and don't actually make the request.
        """
        return self.tnp.get_collections(limit=limit, offset=offset, page=page).url
